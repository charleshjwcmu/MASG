...
#	i <- 1
		...
		AC_MONTH_END[[i]] <- diff(csi_monthend) / lag(csi_monthend)
		...
		file_name = paste(Output_curve,"/",gsub("[.]","-",names(ac_ts_list)[i]),"_MonthlyReturn.png",sep="")
		png(file_name, width=600,height=400)
		ts.plot(AC_MONTH_END[[i]],col=1:3,type="b",main=names(ac_ts_list)[i])
		dev.off()
